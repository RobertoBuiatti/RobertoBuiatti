<h1 align="center">👋 Olá! Eu sou o Roberto Buiatti</h1>

<p align="center">
🚀 Desenvolvedor Full Stack apaixonado por tecnologia, código limpo e soluções criativas.
</p>

<p align="center">
<a href="https://github.com/RobertoBuiatti">
<img src="https://komarev.com/ghpvc/?username=RobertoBuiatti&style=flat-square&color=blue" alt="Visualizações de Perfil" />
</a>
</p>

<p align="center">
<a href="https://robertobuiatti.github.io/pagina-de-gest-o-administrativa/" target="_blank">
<img src="https://img.shields.io/badge/%F0%9F%9A%80%20Ver%20meu%20Portf%C3%B3lio-Online-38bdf8?style=for-the-badge&labelColor=0a0e17" alt="Portfólio Online" />
</a>
<a href="https://play.google.com/store/apps/details?id=com.buiatti.empreendedor&hl=pt_BR" target="_blank">
<img src="https://img.shields.io/badge/%F0%9F%93%B1%20App%20na%20Google%20Play-Empreendedor-34d399?style=for-the-badge&labelColor=0a0e17" alt="App na Google Play" />
</a>
</p>

---

## 👨‍💻 Sobre mim

Sou um desenvolvedor com foco em **JavaScript, React, Node.js** e tecnologias web modernas. Gosto de criar soluções do zero, aprender constantemente e colaborar com pessoas que também curtem tecnologia.

### 💼 Atualmente:

- 📱 Desenvolvendo apps com **React Native + Expo**
- 🖥️ Criando software desktop com **Electron + React**
- 🌐 Criando APIs REST com **Node.js + MongoDB**
- 📊 Explorando **automação e análise de dados com Python**
- 🤝 Aberto a oportunidades e parcerias tech!

---

- 💻 **Frontend**: HTML5, CSS3, JavaScript (ES6+), React
- ⚙️ **Backend**: Node.js, Express.js
- 🛢️ **Banco de Dados**: MongoDB, MySQL (básico)
- 📱 **Mobile**: React Native com Expo
- 🖥️ **Desktop**: Electron
- 🧪 **Testes**: Jest (em andamento)
- 🌐 **APIs**: RESTful, integração com Firebase
- 🔧 **Ferramentas**: Git, GitHub, VS Code, Postman, Figma

---

## 🛠️ Tecnologias & Ferramentas

<div align="center">
<img src="https://skillicons.dev/icons?i=js,ts,react,nodejs,html,css,python,c,mongodb,mysql,express,jest,firebase,git,github,vscode,postman,figma" />
</div>

---

## 📈 Estatísticas GitHub

<div align="center">
<img height="180em" src="https://github-readme-stats.vercel.app/api?username=RobertoBuiatti&show_icons=true&theme=github_dark&include_all_commits=true&count_private=true" alt="Estatísticas do GitHub"/>
<img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=RobertoBuiatti&layout=compact&langs_count=8&theme=github_dark" alt="Linguagens mais usadas"/>
</div>

---

## 🌐 Onde me encontrar

<div align="center">
<a href="https://www.instagram.com/roberto.buiatti/" target="_blank">
<img src="https://img.shields.io/badge/-Instagram-%23E4405F?style=for-the-badge&logo=instagram&logoColor=white">
</a>
<a href="mailto:robertobuiatti2@gmail.com">
<img src="https://img.shields.io/badge/-Gmail-%23333?style=for-the-badge&logo=gmail&logoColor=white">
</a>
<a href="https://www.linkedin.com/in/roberto-buiatti-10b403143" target="_blank">
<img src="https://img.shields.io/badge/-LinkedIn-%230077B5?style=for-the-badge&logo=linkedin&logoColor=white">
</a>
</div>

---

## 📌 Projetos em Destaque

### ⭐ [Empreendedor](https://play.google.com/store/apps/details?id=com.buiatti.empreendedor&hl=pt_BR) *(Publicado na Google Play)*
> App completo de gestão para autônomos e empreendedores individuais, com agenda, CRM, estoque e finanças em um só lugar.

- 📅 Agenda inteligente, CRM com clientes VIP e lembretes via WhatsApp
- 💰 Gestão financeira (DRE), controle de estoque e dashboard em tempo real
- 🌍 8 idiomas, funcionamento offline, backup na nuvem e conformidade LGPD

---

### 🖥️ SecureVision Local *(Software Desktop – Windows)*
> Software que transforma o PC em uma central de monitoramento (NVR/VMS) para câmeras WiFi/IP, 100% offline e sem nuvem.

- 🎥 Descoberta de câmeras ONVIF/RTSP e gravação 24/7 local
- 🧠 Detecção de pessoas/veículos com ML local (ONNX/TensorFlow.js)
- ⚙️ Stack: **Electron + React 19 + TypeScript + FFmpeg + SQLite**
- 🔐 Repositório privado (GitHub)

---

### 🍽️ Gestão – ERP para Restaurantes
> Sistema completo de gestão com multiestoque, dashboard BI e integrações de pagamento.

- 💳 Integração PIX (Mercado Pago) e iFood
- ⚙️ Stack: **Node.js + TypeScript + Prisma + PostgreSQL + React**
- 🔐 Repositório privado (GitHub)

---

### 🔗 [Encurtador de Links](https://github.com/RobertoBuiatti/Encurtador_link)
> Aplicação full stack de encurtamento de URLs, com arquitetura modular e escalável.

- ⚙️ Stack: **React + TypeScript + Vite + Node.js + Express**
- 🧩 Código totalmente tipado e componentizado

---

### 🎙️ [Assistente de Voz com IA](https://github.com/RobertoBuiatti/Auxilio-de-tela-por-voz)
> Assistente que captura voz e tela, analisa imagens com IA (Gemini) e responde por áudio em português.

- 🐍 **Python** + API Gemini + síntese de voz (TTS)
- ⚡ Cache, rate limiting e otimização de imagens

---

### 🔹 [Merge-Sorte-Bidirecional-Buiatti](https://github.com/RobertoBuiatti/Merge-Sorte-Bidirecional-Buiatti)
> Implementação personalizada do algoritmo Merge Sort com abordagem bidirecional.

- 🧠 Foco em lógica e performance
- 💡 Ideal para estudos de algoritmos em **Python**

---

### 🔹 [Busca-Binaria-Buiatti](https://github.com/RobertoBuiatti/Busca-Binaria-Buiatti)
> Algoritmo de busca binária com exemplos práticos e explicações.

- 📚 Estrutura didática
- 👨‍🏫 Perfeito para iniciantes/intermediários em Python

---

## ⭐️ Se curtir algum projeto, deixa uma estrela!
Isso me ajuda muito e me motiva a continuar criando coisas legais 🚀
